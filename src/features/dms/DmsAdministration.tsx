...
const configTabs: ConfigTab[] = ["templates", "workflows", "books", "catalogs"];
...
export const DmsAdministration: React.FC<{ principal: DmsPrincipal }> = ({ principal }) => {
  ...
  React.useEffect(() => {
    ...
    void request
      ...
      .catch((cause: unknown) => { if (active) setError(administrationError(cause, t)); })
  ...
  const selectTab = (nextTab: ConfigTab): void => { setLoading(true); setTab(nextTab); setError(null); };
  const handleTabKeyDown = (event: React.KeyboardEvent<HTMLDivElement>): void => {
    if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
    event.preventDefault();
    const index = configTabs.indexOf(tab);
    const nextIndex = event.key === 'Home' ? 0 : event.key === 'End' ? configTabs.length - 1 : (index + (event.key === 'ArrowRight' ? 1 : -1) + configTabs.length) % configTabs.length;
    selectTab(configTabs[nextIndex]);
  ...
  return (
    <div className="p-4 sm:p-5" data-testid="dms-administration">
      ...
      <div className="mt-4 flex overflow-x-auto border-b border-border" role="tablist" onKeyDown={handleTabKeyDown}>{configTabs.map((item) => <button key={item} id={`dms-admin-tab-${item}`} type="button" role="tab" aria-controls={`dms-admin-panel-${item}`} aria-selected={tab === item} tabIndex={tab === item ? 0 : -1} onClick={() => selectTab(item)} className={`min-h-11 shrink-0 border-b-2 px-4 text-sm font-semibold ${tab === item ? "border-primary text-primary" : "border-transparent text-text-secondary"}`}>{t(`admin.tabs.${item}`)}</button>)}</div>
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(300px,38%)]">
        <section id={`dms-admin-panel-${tab}`} role="tabpanel" aria-labelledby={`dms-admin-tab-${tab}`} tabIndex={0} className="overflow-hidden rounded-lg border border-border">
...
const administrationError = (cause: unknown, t: (key: string) => string): string => {
  if (cause instanceof DmsApiError && cause.status === 403) return t("admin.readOnlyUnavailable");
  if (cause instanceof DmsApiError && cause.status === 401) return t("errors.oauth");
