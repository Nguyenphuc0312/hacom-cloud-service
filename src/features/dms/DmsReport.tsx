...
type ReportFilters = { from?: string; to?: string; direction?: string; state?: string; documentType?: string; processorScope?: "ME" };
...
export const DmsReport: React.FC<{ canExport?: boolean; onDrillDown: (group: ReportGroup, filters: ReportFilters) => void }> = ({ canExport = false, onDrillDown }) => {
  ...
  const [onlyMyProcessing, setOnlyMyProcessing] = React.useState(false);
  ...
  const selectedFilters = (): ReportFilters => ({ from: from || undefined, to: to || undefined, direction: direction || undefined, state: state || undefined, documentType: documentType.trim() || undefined, processorScope: onlyMyProcessing ? "ME" : undefined });
