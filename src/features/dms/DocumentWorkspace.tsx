...
const ActionPanel: React.FC<{ document: DmsDocument; mode: ActionMode; onClose: () => void; onDone: () => void }> = ({ document, mode, onClose, onDone }) => {
  ...
  return <form onSubmit={(event) => void submit(event)} className="border-b border-border bg-primary/5 p-4" aria-labelledby="dms-action-panel-title">
    <div className="mb-3 flex justify-between"><h4 id="dms-action-panel-title" className="font-bold text-text-primary">{t(`actionPanel.${mode}`)}</h4><IconButton icon={<XMarkIcon />} aria-label={t("actions.close")} size="sm" onClick={onClose} /></div>
