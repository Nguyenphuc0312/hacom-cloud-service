...
export const CloudChatWorkspace: React.FC = () => {
  ...
  const refresh = useCallback(async () => {
    try {
      const [nextSpace, nextAssets] = await Promise.all([
        cloudApi.ensure(),
        cloudApi.list({ q: query || undefined }),
      ]);
      setSpace(nextSpace);
  ...
  const recentMedia = useMemo(() => assets.filter((asset) => ['image', 'video'].includes(asset.mediaType)).slice(0, 6), [assets]);
  const recentFiles = useMemo(() => assets.filter((asset) => !['image', 'video'].includes(asset.mediaType)).slice(0, 5), [assets]);
  ...
  return <section className="flex h-full min-h-0 overflow-hidden bg-surface text-text-primary">
    <aside className="hidden w-[var(--app-sidebar-width)] shrink-0 border-r border-border/70 bg-surface md:flex md:flex-col" aria-label="Hacom Cloud">
      ...
      <button type="button" className="mx-2 flex items-center gap-3 rounded-lg bg-surface-active px-3 py-3 text-left" aria-current="page"><span className="grid h-9 w-9 place-items-center rounded-full bg-brand-soft text-brand-solid"><CloudIcon className="h-5 w-5" /></span><span className="min-w-0"><strong className="block truncate text-sm">Cloud của tôi</strong><small className="block truncate text-text-muted">{assets[0]?.originalFilename ?? 'Lưu trữ riêng tư'}</small></span></button>
    </aside>
    <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
      <header className="chat-header shrink-0 border-b border-border/70 bg-surface"><ConversationLane><div className="chat-header-row flex min-h-[var(--app-header-height)] items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-brand-soft text-brand-solid"><CloudIcon className="h-5 w-5" /></span><div className="min-w-0 flex-1"><h1 className="truncate text-[15px] font-medium">Cloud của tôi</h1><p className="truncate text-xs text-text-muted">Lưu trữ và đồng bộ dữ liệu giữa các thiết bị</p></div><button type="button" className="chat-header-action inline-flex h-9 w-9 items-center justify-center rounded-md hover:bg-surface-hover" onClick={() => setInfoOpen((open) => !open)} aria-label="Bật hoặc tắt thông tin Hacom Cloud"><PanelLeft className="h-5 w-5" /></button></div></ConversationLane></header>
