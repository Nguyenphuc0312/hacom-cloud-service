import { adminAxiosInstance } from '@/api/axios/axios';
import { unwrapApiEnvelope } from '@/api/envelope/envelope';
import type { SendTestEmailRequest, SmtpSettingsPayload, TestConnectionResponse, UpdateSmtpSettingsRequest } from '@/api/types/smtp/smtp';

export const smtpClient = {
  async getSettings(): Promise<SmtpSettingsPayload> {
    const response = await adminAxiosInstance.get('/settings/smtp');
    return unwrapApiEnvelope<SmtpSettingsPayload>(response);
  },

  async updateSettings(payload: UpdateSmtpSettingsRequest): Promise<SmtpSettingsPayload> {
    const response = await adminAxiosInstance.put('/settings/smtp', payload);
    return unwrapApiEnvelope<SmtpSettingsPayload>(response);
  },

  async activateDraft(): Promise<SmtpSettingsPayload> {
    const response = await adminAxiosInstance.post('/settings/smtp/activate');
    return unwrapApiEnvelope<SmtpSettingsPayload>(response);
  },

  async deactivateActive(): Promise<SmtpSettingsPayload> {
    const response = await adminAxiosInstance.post('/settings/smtp/deactivate');
    return unwrapApiEnvelope<SmtpSettingsPayload>(response);
  },

  async testConnection(payload?: UpdateSmtpSettingsRequest): Promise<TestConnectionResponse> {
    const response = await adminAxiosInstance.post('/settings/smtp/test-connection', payload);
    return unwrapApiEnvelope<TestConnectionResponse>(response);
  },

  async sendTestEmail(payload: SendTestEmailRequest): Promise<TestConnectionResponse> {
    const response = await adminAxiosInstance.post('/settings/smtp/send-test-email', payload);
    return unwrapApiEnvelope<TestConnectionResponse>(response);
  },
};
