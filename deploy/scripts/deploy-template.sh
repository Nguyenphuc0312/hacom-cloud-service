#!/usr/bin/env bash
# Standardized deploy script template
# Usage: ./deploy.sh
#
# Required .release.env variables:
#   DEPLOY_ENV          - Environment name (develop|staging|production)
#   SERVICE_NAME        - Service identifier
#   SERVER_RUNTIME_ENV_FILE  - Absolute path to runtime env file on server
#   IMAGE_REF           - Full image reference (e.g., ghcr.io/owner/service:sha)
#   COMPOSE_FILE        - Relative path to compose file
#   COMPOSE_PROJECT_NAME - Docker compose project name
#   RUNTIME_SERVICE     - Docker compose service name
#
# Optional .release.env variables:
#   HEALTHCHECK_URL     - HTTP URL for health check
set -Eeuo pipefail
...
# Check runtime env file exists before creating symlink
