...
// DefaultLimitBytes is the current product proposal (5 decimal GB).
// Keep this overridable until the mentor confirms the quota policy.
const DefaultLimitBytes int64 = 5_000_000_000
...
func WithDefaultLimitBytes(limitBytes int64) Option {
	return func(service *Service) {
		if limitBytes > 0 {
			service.limitBytes = limitBytes
		}
	}
}
...
func (s *Service) EnsureDefaultQuota(ctx context.Context, ownerID string) (Usage, error) {
	...
	return s.repository.EnsureQuota(ctx, ownerID, s.limitBytes)
}
