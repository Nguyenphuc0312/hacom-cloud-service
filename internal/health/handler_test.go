...
func TestReadinessEnforcesTimeoutWhenCheckerDoesNotObserveContext(t *testing.T) {
	timeout := 10 * time.Millisecond
	handler := NewHandler(NewService(
		timeout,
		fakeChecker{name: "blocked", wait: time.Second},
	))
	...
	if elapsed := time.Since(startedAt); elapsed > 200*time.Millisecond {
		t.Fatalf("health check took %s, expected timeout near %s", elapsed, timeout)
	}
	if response.Code != http.StatusServiceUnavailable {
