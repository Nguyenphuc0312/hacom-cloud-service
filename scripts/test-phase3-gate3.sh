...
for command in docker migrate go node; do
...
echo "[2/7] Applying all migrations and verifying the Cloud schema"
migrate -path migrations -database "$TEST_DATABASE_URL" up
compose exec -T postgres psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" \
  -d "$TEST_DB_NAME" -f /dev/stdin < scripts/verify-schema.sql
...
echo "[6/7] Vetting and building Cloud API and Worker"
go vet ./...
go build ./cmd/api ./cmd/worker

echo "[7/7] Validating the external HTTP handoff collection JSON"
node -e "JSON.parse(require('fs').readFileSync('tests/postman/Hacom-Cloud-Phase-3-Gate-3.postman_collection.json','utf8'))"

echo "Phase 3 Gate 3 Cloud scope PASSED. External Auth/Admin/Notification backend integration remains DEFERRED."
